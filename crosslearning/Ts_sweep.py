...
def argNdatasets(dictstarts, dictmids, N):
    """
    Generate datasets for a given N using the provided start and mid points.
    """
    dictmids['ARG'] = dictstarts['ARG'] + N
    dictstops = {}
    for key in dictstarts.keys():
    ...
    return get_SIR_covid_datasets(countries, starts, mids, stops)

# Example usage

datasets2 = argNdatasets(dictstarts, dictmids, N=15)
...
Nsarg = np.arange(2,len(modelargfull.I_data))
...
for Narg in tqdm(Nsarg, desc="Ns Progress"):
    Niter = Niter_short
    datasets = argNdatasets(dictstarts, dictmids, Narg)

    # Update models with new datasets
    models = {}
    for country in countries:
        y0 = datasets[country]['train']['array'][:, 0]
        if country is not 'ARG':
            I_data, _ = cutdata(datasets[country]['train']['array'][1, :], proportion=0.8)
        else:
            I_data = datasets[country]['train']['array'][1, :]  # For ARG, use all data without cutting
        models[country] = SIRModel(y0, I_data, country=country)

    # x, z, y, w = admm_optimize(models, epsilon, Niter, initial_guess, lda)
    ...
    # if epsilon==-1:
    #     plot_params(models,z,y,w,epsilon+1e-6)
    ...
    global_params = minimize_global_loss(models) #cent line
    modelargfull.beta = global_params[0] #cent line
    modelargfull.gamma = global_params[1] #cent line
    ...
    print(f"N: {Narg}, Peak error: {peak_error_}, Lag error: {lag_error_}")
...
df = pd.DataFrame({'Ns': Nsarg, 'peak_error': peak_errors, 'lag_error': lag_errors, 'peak_errors_log': np.log(peak_errors), 'lag_errors_log': np.log(lag_errors)})
df.to_csv(f'results_vs_N_{timestamp}.csv', index=False)


# Peak error plot
plt.figure(figsize=(10, 6))
#plt.plot(0, peak_error_cent, marker='o')
plt.plot(Nsarg, peak_errors, marker='o', linestyle='-', color='blue', label='Peak error')
...
plt.plot(Nsarg, lag_errors, marker='o', linestyle='-', color='blue', label='Lag error')
...
plt.plot(Nsarg, np.log(lag_errors), marker='o', linestyle='-', color='blue', label='Lag error')
